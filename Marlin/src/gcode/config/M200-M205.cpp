...
void GcodeSuite::M204() {
  if (!parser.seen("PRSTH")) {
    ...
    SERIAL_ECHOLNPGM_P(
        "Acceleration: P", planner.settings.acceleration,
        " R", planner.settings.retract_acceleration,
        SP_T_STR, planner.settings.travel_acceleration,
        " H", planner.settings.XYhoming_acceleration
      );
  }
  else {
    //planner.synchronize();
